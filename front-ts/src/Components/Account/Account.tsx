  ...
  return (
    <div className='glavDiv'>
      ...
      <form onSubmit={handleSubmit(poiskCard)}>
        <input {...register('poisk')} className='divSearch' placeholder=' 🔍  поиск карт'/>
        <button className='divSearch'>✔</button>
      </form>
      </div>
       <h3 className='animate__animated animate__zoomInUp'>Привет {state.userReducer.user.login.toUpperCase()}!</h3>
      <DeckList />
    </div>
  );
}
